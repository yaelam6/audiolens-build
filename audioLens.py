...
class App(ctk.CTk):
    ...
    def _select_known(self, rec):
        """Select a mix-match result whose BPM/key are already known — no re-analysis."""
        bpm  = rec["match_bpm"]
        key  = rec["match_key"]
        mode = rec["match_mode"]
        self.song_lbl.configure(text=f"{rec['name']} — {rec['artist']}")
        ...
        self._set_ref(bpm, key, mode, rec.get("genre", ""), rec.get("artist", ""), rec.get("track_id", ""))
    ...
    def _add_match_card(self, rec, mg):
        ...
        ctk.CTkButton(
            ...
            font=("Arial", 10, "bold"),
            command=lambda rx=rec: self._select_known(rx),
